...
class Download(Screen):
    ...
    def make_therad(self):
        threading.Thread(target = self.make_search_id).start()
    ...
    def creat_blog(self, title, author, date, thumbnail_url, lenght, id_index):
        ...
        mdLabel1 = MDLabel(text=str(title),font_style="Body1",theme_text_color="Primary",bold=True)
        mdLabel2 = MDLabel(text=str(author),font_style="Body2",theme_text_color="Primary")
    ...
    def download_mp3_method(self,i):
        threading.Thread(target = self.download_mp3,args=[i]).start()


    def download_mp4_method(self,i):
        threading.Thread(target = self.download_mp4,args=[i]).start()
    ...
    def download_mp3(self,id_index):
        try:
            ...
            yt = YouTube(url)
            yt.streams.get_by_itag(140).download(self.adress,"deneme.mp3")
            print("İndirildi")
    ...
    def download_mp4(self,id_index):
        try:
            ...
            url = self.url_list[id_index]
            yt = YouTube(url)
            yt.streams.get_by_itag(18).download(self.adress,"deneme.mp4")
            print("İndirildi")
            self.ids.download_spinner.active = False
